#!/usr/bin/env python3
"""Fetch the emrQA dataset (emr_qa) and convert it
   into JSONL that PreExamChartingAgent's evaluation pipeline can read.

Usage (from repo root):

    python evaluation/scripts/fetch_emrqa_dataset.py \
        --output evaluation/datasets/emrqa_qa.jsonl \
        --sample_size 20000   # optional – keep ≤ 400k
...
def dump_jsonl(ds, output_path: Path, sample_size: Optional[int] = None):
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for row in ds:
            # emrQA has 'question', 'context', 'answer' fields
            obj = {
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=Path("evaluation/datasets/emrqa_qa.jsonl"))
    parser.add_argument("--sample_size", type=int, default=None,
                        help="Limit number of rows (≤400k)")
    args = parser.parse_args()

    print("Downloading 'emr_qa' …")
    ds = load_dataset("emr_qa", split="train")
